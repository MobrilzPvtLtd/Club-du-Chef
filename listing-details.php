...
$check_query = "SELECT day, start_time, end_time, start_time_2, end_time_2 FROM " . TBL . "booking_availability WHERE booking_type_id = '{$listrow['listing_id']}' AND is_available = 1 AND booking_type = 'listing'";
$availability_day_result = mysqli_query($conn, $check_query);

// Fetch existing booking dates from the database
$bookings = "SELECT date_time FROM " . TBL . "bookings WHERE booking_type = 'listing'";
                    ...
                    <?php if ($singleFirst4Images || $singleLast4Images) { ?>
                        <div class="ld-rhs-pro pglist-bg pglist-p-com">
                            ...
                            <section class="logos-slider slider">
                                    <?php
                                        foreach ($singleFirst4Images as $index => $imageUrl) {
                                    $link = isset($singleFirst4ImageLinks[$index]) ? $singleFirst4ImageLinks[$index] : '#';
                                    ?>
                            ...
                            <section class="logos-slider slider">
                                    <?php
                                        foreach ($singleLast4Images as $index => $imageUrl) {
                                    $link = isset($singleLast4ImageLinks[$index]) ? $singleLast4ImageLinks[$index] : '#';
                                    ?>
                                    <a href="<?php echo $link; ?>" target="_blank">
                                        <li>
                                            <div style="padding: 0;">
