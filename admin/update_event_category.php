...
//************ Category Name Already Exist Check Starts ***************
        ...
        $category_name_exist_check = mysqli_query($conn, "SELECT * FROM " . TBL . "event_categories  WHERE category_name='" . $category_name . "' AND category_id != $category_id ");
        ...
        if (mysqli_num_rows($category_name_exist_check) > 0) {
            ...
            header('Location: admin-event-category-edit.php?row=' . $category_id);
            exit;
...
//************ Category Name Already Exist Check Ends ***************
        ...
        function checkEventttCategorySlug($link, $category_id, $counter = 1)
        {
            global $conn;
            $newLink = $link;
            do {
                $checkLink = mysqli_query($conn, "SELECT category_id FROM " . TBL . "event_categories WHERE category_slug = '$newLink' AND category_id != '$category_id'");
        ...
        $category_slug = checkEventttCategorySlug($category_name1, $category_id);

        $sql = mysqli_query($conn, "UPDATE  " . TBL . "event_categories SET category_name='" . $category_name . "', category_status='" . $category_status . "', city_slug='" . $city_slug_json . "'
