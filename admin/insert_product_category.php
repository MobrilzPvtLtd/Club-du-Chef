...
//        function checkProductCategorySlug($link, $counter = 1)
//        {
//            global $conn;
//            $newLink = $link;
//            do {
//                $checkLink = mysqli_query($conn, "SELECT category_id FROM " . TBL . "product_categories WHERE category_slug = '$newLink'");
...
//        $category_slug = checkProductCategorySlug($category_name1);

        $counter = 1;
        $checkLink = mysqli_query($conn, "SELECT category_id FROM " . TBL . "product_categories WHERE category_slug = '$category_name1'");
        if (mysqli_num_rows($checkLink) > 0) {
...
VALUES ('$category_name','$city_slug_json','$category_status','$category_image','$category_filter_pos_id', '$category_slug', '$curDate')");
        ...
        $upqry = mysqli_query($conn, "UPDATE " . TBL . "product_categories 
    ...
    if ($upqry) {
        ...
        $_SESSION['status_msg'] = "Product Category(s) has been Added Successfully!!!";
