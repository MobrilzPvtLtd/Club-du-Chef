...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['product_submit'])) {
        ...
        , product_status='" . $product_status . "', product_slug='" . $product_slug . "', is_booking='" . $is_booking . "', booking_url='" . $booking_url . "' where product_id='" . $product_id . "'";
        ...
        foreach ($days as $day) {
            ...
            if ($is_available == 1) {
                ...
                $check_query = "SELECT * FROM " . TBL . "booking_availability WHERE product_id = '$product_id' AND day = '$day'";
                $result = mysqli_query($conn, $check_query);
        
                if (mysqli_num_rows($result) > 0) {
                    // If the day already exists, update the availability
                    $update_queries[] = "UPDATE " . TBL . "booking_availability SET start_time = '$start_time', end_time = '$end_time', is_available = 1, created_at = '$curDate' WHERE product_id = '$product_id' AND day = '$day'";
                } else {
                    // If the day does not exist, insert a new record
                    $insert_queries[] = "INSERT INTO " . TBL . "booking_availability (product_id, day, start_time, end_time, is_available, created_at) VALUES ('$product_id', '$day', '$start_time', '$end_time', 1, '$curDate')";
                }
            } else {
                // If the checkbox is not checked, set is_available to 0
                $update_queries[] = "UPDATE " . TBL . "booking_availability SET is_available = 0 WHERE product_id = '$product_id' AND day = '$day'";
            }
        }
        ...
        //****************************    client email ends    *************************
            ...
            header("Location: admin-edit-product.php?code=$product_code");
        }
