...
//************ Category Name Already Exist Check Starts ***************
        ...
        $category_name_exist_check = mysqli_query($conn, "SELECT * FROM " . TBL . "expert_categories  WHERE category_name='" . $category_name . "' AND category_id != $category_id ");
        ...
        if (mysqli_num_rows($category_name_exist_check) > 0) {
            ...
            header('Location: expert-category-edit.php?row=' . $category_id);
            exit;
...
//************ Category Name Already Exist Check Ends ***************
        ...
        function checkExpertCategorySlug($link, $category_id, $counter = 1)
        {
            global $conn;
            $newLink = $link;
            do {
                $checkLink = mysqli_query($conn, "SELECT category_id FROM " . TBL . "expert_categories WHERE category_slug = '$newLink' AND category_id != '$category_id'");
        ...
        if ($sql) {
            ...
            $_SESSION['status_msg'] = "Expert Service category has been Updated Successfully!!!";
