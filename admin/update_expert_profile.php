     ...
     , expert_udt ='" . $curDate . "', expert_status='" . $expert_status . "', expert_slug='" . $expert_slug . "', is_booking='" . $is_booking . "', booking_url='" . $booking_url . "'
       ...
       foreach ($days as $day) {
           ...
           if ($is_available == 1) {
               ...
               $check_query = "SELECT * FROM " . TBL . "booking_availability WHERE expert_id = '$expert_id' AND day = '$day'";
               $result = mysqli_query($conn, $check_query);
       
               if (mysqli_num_rows($result) > 0) {
                   // If the day already exists, update the availability
                   $update_queries[] = "UPDATE " . TBL . "booking_availability SET start_time = '$start_time', end_time = '$end_time', is_available = 1, created_at = '$curDate' WHERE expert_id = '$expert_id' AND day = '$day'";
               } else {
                   // If the day does not exist, insert a new record
                   $insert_queries[] = "INSERT INTO " . TBL . "booking_availability (expert_id, day, start_time, end_time, is_available, created_at) VALUES ('$expert_id', '$day', '$start_time', '$end_time', 1, '$curDate')";
               }
           } else {
               // If the checkbox is not checked, set is_available to 0
               $update_queries[] = "UPDATE " . TBL . "booking_availability SET is_available = 0 WHERE expert_id = '$expert_id' AND day = '$day'";
           }
       }
