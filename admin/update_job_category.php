...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['category_submit'])) {
        ...
        if (mysqli_num_rows($category_name_exist_check) > 0) {
            ...
            header('Location: admin-job-category-edit.php?row=' . $category_id);
            exit;
        ...
        function checkJobbbCategorySlug($link, $category_id, $counter = 1)
        {
            global $conn;
            $newLink = $link;
            do {
                $checkLink = mysqli_query($conn, "SELECT category_id FROM " . TBL . "job_categories WHERE category_slug = '$newLink' AND category_id != '$category_id'");
