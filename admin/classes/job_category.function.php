...
function getAllJobCategoriesOrderByJobsTable()
{
    ...
    $sql = "SELECT T1.*, COUNT(T2.category_id) AS views FROM " . TBL . "categories AS T1 LEFT JOIN " . TBL . "jobs AS T2 ON T1.category_id = T2.category_id GROUP BY T1.category_id ORDER BY views DESC";
    $rs = mysqli_query($conn, $sql);
    return $rs;
...
function getAllJobCategoriesOrderByJobsTableLimit($arg)
{
    ...
    $sql = "SELECT T1.*, COUNT(T2.category_id) AS views FROM " . TBL . "categories AS T1 LEFT JOIN " . TBL . "jobs AS T2 ON T1.category_id = T2.category_id AND (JSON_CONTAINS(T1.city_slug, '\"$CurrentCity\"') OR '$CurrentCity' = 'www') GROUP BY T1.category_id ORDER BY views DESC LIMIT $arg";
    $rs = mysqli_query($conn, $sql);
    return $rs;
...
function getAllJobCategoriesLimit($arg)
{
    ...
    $sql = "SELECT * FROM " . TBL . "categories ORDER BY category_id DESC LIMIT $arg";
    $rs = mysqli_query($conn, $sql);
    return $rs;
