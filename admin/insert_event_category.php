...
// *********** if Count of category name is zero means redirect starts ********
    ...
    if ($cnt == 0) {
        header('Location: admin-add-new-event-category.php');
        exit;
    }
...
//        function checkEventCategorySlug($link, $counter = 1)
//        {
//            global $conn;
//            $newLink = $link;
//            do {
...
//        $category_slug = checkEventCategorySlug($category_name1);
        ...
        $sql = mysqli_query($conn, "INSERT INTO  " . TBL . "event_categories (category_name,city_slug,category_status,category_image,category_filter_pos_id,category_slug,category_cdt)
VALUES ('$category_name','$city_slug_json','$category_status','$category_image','$category_filter_pos_id', '$category_slug', '$curDate')");
    ...
    if ($upqry) {
        ...
        $_SESSION['status_msg'] = "Event Category(s) has been Added Successfully!!!";
